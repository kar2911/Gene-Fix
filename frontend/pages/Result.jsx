...
const Result = () => {
  ...
  useEffect(() => {
    ...
    const fetchResults = async () => {
      const response = await fetch("http://localhost:8000/gene-fix", {
        method: "POST",
        headers: {
  ...
  const { gene, mutation, mut_type, repair, trials, wild_url, mutated_url } =
    resultData
  const wildTypeUrl =
    wild_url || "http://localhost:8000/static/TP53/TP53_wild.pdb"
  const mutantUrl =
    mutated_url ||
    "http://localhost:8000/static/TP53/p.R175H/TP53_pr175h_missense.pdb"

  return (
    <div className="p-6 max-w-6xl mx-auto">
      <h2 className="text-2xl font-bold mb-2">
        Mutation Report: {gene} – {mutation}
      </h2>
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        ...
        <p className="text-sm text-gray-600 mt-2">
          Use the slider to transition between wild type and mutant protein
          structures.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <pre className="bg-gray-100 p-4 rounded overflow-auto max-h-60">
            {repair || "No CRISPR repair plan available."}
          </pre>
        </div>
            ...
            <div className="space-y-3 overflow-auto max-h-60">
              {trials.map((t, i) => (
                <div key={i} className="bg-gray-50 p-3 rounded shadow-sm">
                  ...
                  <p>
                    <strong>Title:</strong> {t["Study Title"]}
                  </p>
                </div>
              ))}
            </div>
